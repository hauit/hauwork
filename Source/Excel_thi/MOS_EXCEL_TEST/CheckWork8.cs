...
// Type: MOS_EXCEL_TEST.CheckWork8
...
namespace MOS_EXCEL_TEST
{
  public static class CheckWork8
  {
    ...
    private static string Cau0(Application a, Workbook d)
    {
      ...
      try
      {
        worksheet = (Worksheet) d.Worksheets[(object) "January"];
      }
      catch (Exception ex)
      {
    ...
    private static string Cau1(Application a, Workbook d)
    {
      ...
      try
      {
        worksheet = (Worksheet) d.Worksheets[(object) "March"];
      }
      catch (Exception ex)
      {
      ...
      try
      {
        if (listObject.AutoFilter.Filters[6].Criteria1.ToString() != "=MP")
          return "False";
      }
      catch (Exception ex)
      {
    ...
    private static string Cau2(Application a, Workbook d)
    {
      try
      {
        Worksheet worksheet = (Worksheet) d.Worksheets[(object) "February"];
        if (!worksheet.get_Range((object) "G5", (object) "G5").Formula.ToString().Contains("=IF([@[Years as Member]]>3,\"Yes\",\"No\")"))
          return "False(=IF([@[Years as Member]]>3,\"Yes\",\"No\"))";
        if (!worksheet.get_Range((object) "G18", (object) "G18").Formula.ToString().Contains("=IF([@[Years as Member]]>3,\"Yes\",\"No\")"))
          return "False(=IF([@[Years as Member]]>3,\"Yes\",\"No\"))";
      }
      catch (Exception ex)
      {
    ...
    private static string Cau3(Application a, Workbook d)
    {
      ...
      {
        Worksheet worksheet = (Worksheet) d.Worksheets[(object) "February"];
        if (!worksheet.get_Range((object) "F5", (object) "F5").Formula.ToString().Contains("=LEFT([@[Policy Number ]],2)"))
          return "False(=LEFT([@[Policy Number ]],2))";
        if (!worksheet.get_Range((object) "F18", (object) "F18").Formula.ToString().Contains("=LEFT([@[Policy Number ]],2)"))
          return "False(=LEFT([@[Policy Number ]],2))";
      }
      catch (Exception ex)
      {
