...
// Type: MOS_EXCEL_TEST.CheckWork23
...
namespace MOS_EXCEL_TEST
{
  public static class CheckWork23
  {
    ...
    private static string Cau0(Application a, Workbook d)
    {
      try
      {
        Worksheet worksheet = (Worksheet) d.Worksheets[(object) "Adult Program"];
        if (worksheet.get_Range((object) "H10", (object) "H19").SparklineGroups.Count != 1)
          return "False(chen Sparkline)";
        if (worksheet.get_Range((object) "H10", (object) "H19").SparklineGroups[(object) 1].SourceData != "B10:F19")
          return "False(B10:F19)";
        if (worksheet.get_Range((object) "H10", (object) "H19").SparklineGroups[(object) 1].Type != XlSparkType.xlSparkColumn)
          return "False(Column)";
      }
      catch (Exception ex)
      {
    ...
    private static string Cau1(Application a, Workbook d)
    {
      try
      {
        if (((_Worksheet) d.Worksheets[(object) "Youth Program"]).ListObjects.Count == 0)
    ...
    private static string Cau2(Application a, Workbook d)
    {
      try
      {
        if (((_Worksheet) d.Worksheets[(object) "Summary"]).Visible != XlSheetVisibility.xlSheetVisible)
          return "False(bo an trang tinh Summary)";
      }
      catch (Exception ex)
      {
        return "False(something wrong)";
    ...
    private static string Cau3(Application a, Workbook d)
    {
      try
      {
        ...
        if (worksheet.ListObjects[(object) 1].AlternativeText != "Adult Enrollment")
          return "False(Adult Enrollment)";
      }
      catch (Exception ex)
      {
        return "False(something wrong)";
