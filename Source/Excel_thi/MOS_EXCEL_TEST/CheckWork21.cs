...
// Type: MOS_EXCEL_TEST.CheckWork21
...
namespace MOS_EXCEL_TEST
{
  public static class CheckWork21
  {
    ...
    private static string Cau0(Application a, Workbook d)
    {
      ...
      try
      {
        worksheet = (Worksheet) d.Worksheets[(object) "Summer Bookings"];
      }
      catch (Exception ex)
      {
        return "Fales (Summer Bookings)";
      }
      try
      {
        if (!worksheet.get_Range((object) "M9", (object) "M9").Formula.ToString().Contains("=COUNTIF(Table1[Group Size],\">=12\")"))
          return "False(=COUNTIF(Table1[Group Size],\">=12\"))";
      }
      catch (Exception ex)
      {
    ...
    private static string Cau1(Application a, Workbook d)
    {
      ...
      try
      {
        if (!worksheet.get_Range((object) "M10", (object) "M10").Formula.ToString().Contains("=SUMIF(Table1[Group Size],\">=12\",Table1[Total])"))
          return "False(=SUMIF(Table1[Group Size],\">=12\",Table1[Total]))";
      }
      catch (Exception ex)
      {
        return "Something wrong";
    ...
    private static string Cau2(Application a, Workbook d)
    {
      ...
      try
      {
        if (!worksheet.get_Range((object) "C8", (object) "C8").Formula.ToString().Contains("=CONCATENATE([@Lastname],\", \",[@Firstname])"))
          return "False(=CONCATENATE([@Lastname],\", \",[@Firstname]))";
      }
      catch (Exception ex)
      {
        return "Something wrong";
    ...
    private static string Cau3(Application a, Workbook d)
    {
      ...
      try
      {
        worksheet = (Worksheet) d.Worksheets[(object) "Spring Bookings"];
      }
      catch (Exception ex)
      {
        return "Fales (Spring Bookings)";
      }
      return worksheet.ListObjects.Count != 0 ? "False (chuyen table thanh day o)" : "True";
    }
    ...
    private static string Cau4(Application a, Workbook d)
    {
      ...
      try
      {
        if (worksheet.PageSetup.CenterFooter != "Page &P of &N")
          return "False(chen vao footer sai kieu)";
      }
      catch (Exception ex)
      {
