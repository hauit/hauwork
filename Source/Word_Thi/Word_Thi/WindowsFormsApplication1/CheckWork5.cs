...
namespace MOS_WORD_TEST
{
    public static class CheckWork5
    {
        ...
        private static string Cau12(Application a, Document d)
        {
            ...
            {
                int Index = 1;
                while (Index < d.Paragraphs.Count && d.Paragraphs[Index].Range.Text != "An eruption\r")
                    ++Index;
                if (Index >= d.Paragraphs.Count)
                ...
                while (Index < d.Paragraphs.Count && d.Paragraphs[Index].Range.Text != "Example\r")
                    ++Index;
                if (Index >= d.Paragraphs.Count)
                    return "False(Text was modyfied)";
                ...
                while (Index < d.Paragraphs.Count && d.Paragraphs[Index].Range.Text != "Location\r")
                    ++Index;
                if (Index >= d.Paragraphs.Count)
                    return "False(Text was modyfied)";
                var style3 = d.Paragraphs[Index].get_Style() as Style;
                if (style3 == null || style3.NameLocal != "Heading 2")
        ...
        private static string Cau15(Application a, Document d)
        {
            try
            {
                foreach (Microsoft.Office.Interop.Word.Table table in d.Tables)
                {
                    ...
                    if (style != null && style.NameLocal == "Grid Table 4")
                    {
                        ...
                        return (double)width > 200.0 ? "False(" + width + ")" : "True";
                    }
                }
            }
            catch
