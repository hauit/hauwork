    ...
    internal class S9_Section
    {
        ...
        private static string cau5(Application a, Presentation d)
        {
            try
            {
                ...
                var namedShow = a.ActivePresentation.SlideShowSettings.NamedSlideShows[(object)1];
                if (namedShow.Name != "Internal")
                    return "False (SlideShow name)";
                count = namedShow.Count;
                if (count.ToString() != "4")
                    return "False (Number of slide in slide show)";
                // SlideIDs is a collection of slide IDs (should be an array)
                var slideIDsObj = namedShow.SlideIDs;
                if (!(slideIDsObj is Array slideIDs) || slideIDs.Length < 5)
                    return "False (SlideIDs not found or too short)";
                // 5th element (index 4) should be 257
                if (slideIDs.GetValue(4)?.ToString() != "257")
                    return "False (Strategy 2020 slide to end)";
                return "True";
            }
            catch (Exception)
            {
        ...
        private static string cau8(Application a, Presentation d)
        {
            try
            {
                ...
                if (count.ToString() != "3")
                    return "False (Number of slide in slide show)";
                var slideIDsObj = namedShow.SlideIDs;
                if (!(slideIDsObj is Array slideIDs) || slideIDs.Length < 4)
                    return "False (SlideIDs not found or too short)";
                if (slideIDs.GetValue(1)?.ToString() != "259")
                    return "False (3)";
                if (slideIDs.GetValue(2)?.ToString() != "260")
                    return "False (4)";
                return slideIDs.GetValue(3)?.ToString() != "261" ? "False (5)" : "True";
            }
            catch (Exception)
            {
                return "False (Something Wrong)";
